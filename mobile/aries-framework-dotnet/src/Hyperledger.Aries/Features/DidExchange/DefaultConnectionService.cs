...
namespace Hyperledger.Aries.Features.DidExchange
{
    ...
    public class DefaultConnectionService : IConnectionService
    {
        ...
        // Author: Kevin Dinh
        // Erstellt eine Einladung die einen BLE Identifikator als Endpunkt hat.
        // Identisch zu der CreateInvitation Funktion mit dem Unterschied das der Endpunkt einen BLE Identifikator enthält.
        public async Task<(ConnectionInvitationMessage, ConnectionRecord)> CreateBleInvitationAsync(IAgentContext agentContext,
            string bleIdentifier, InviteConfiguration config = null)
        {
            var connectionId = !string.IsNullOrEmpty(config?.ConnectionId)
                ? config.ConnectionId
                : Guid.NewGuid().ToString();
        ...
        public async Task<(ConnectionRequestMessage, ConnectionRecord)> CreateBleRequestAsync(IAgentContext agentContext, ConnectionInvitationMessage invitation,
            string bleEndpoint)
        {
            Logger.LogInformation(LoggingEvents.AcceptInvitation, "Key {0}, Endpoint {1}",
                invitation.RecipientKeys[0], invitation.ServiceEndpoint);
            ...
            request.Connection.DidDoc.Services[0].ServiceEndpoint = bleEndpoint;
            ((IndyAgentDidDocService) request.Connection.DidDoc.Services[0]).RoutingKeys = new List<string>();
        ...
        // Author: Kevin Dinh
        // Erstellt eine BLE Response Nachricht die auf eine BLE Request Nachricht antwortet.
        // Identisch zu der CreateResponse Funktion mit dem Unterschied das der Endpunkt einen BLE Identifikator enthält.
        public async Task<(ConnectionResponseMessage, ConnectionRecord)> CreateBleResponseAsync(IAgentContext agentContext, string connectionId, string endpoint)
        {
            ...
            connectionData.DidDoc.Services[0].ServiceEndpoint = endpoint;
